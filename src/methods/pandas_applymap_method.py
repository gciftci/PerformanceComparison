# src/methods/pandas_applymap_method.py
"""
pandas_applymap_method.py: A module that implements the pandas applymap method for processing 2D
arrays.
...
Example usage:
    ...
    from src.methods import pandas_applymap_method

    start_array = ...  # Your 2D input array
    end_array = ...    # Your 3D output array

    # Process the input array using the pandas applymap method
    processed_array = pandas_applymap_method.process(start_array, end_array)

    # Benchmark the performance of the pandas applymap method (timeit)
    timer = Timer(lambda: process(start_array, end_array), setup=setup_code, globals={**_NS})

Note: The `process` function in this module benefits from the efficiency of the pandas `applymap`
function, as it does not require any explicit loops to perform the required operations on
the input array.
"""
...
def setup() -> str:
    """
    Set up code for pandas applymap method.
...
def process(start_array: ndarray, end_array: ndarray) -> ndarray:
    """
    Iterate through the start_array using the pandas `applymap` function and apply filter and
    operation.
    ...
    Benefits: Can be more efficient due to the use of pandas `applymap` function; no explicit loops
    required.
    """
    start_df = DataFrame(start_array)
    end_df = start_df.applymap(lambda x: x * 255 if x > 0 else 0)
    end_array[:, :, 0] = end_df.to_numpy()
    return end_array
