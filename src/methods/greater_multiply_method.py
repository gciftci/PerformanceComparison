# src/methods/greater_multiply_method.py
"""
greater_multiply_method.py: A module that implements the greater multiply method for processing 2D
arrays.
...
Example usage:
    ...
    from src.methods import greater_multiply_method

    start_array = ...  # Your 2D input array
    end_array = ...    # Your 3D output array

    # Process the input array using the greater multiply method
    processed_array = greater_multiply_method.process(start_array, end_array)

    # Benchmark the performance of the greater multiply method (timeit)
    timer = Timer(lambda: greater_multiply_method.process(start_array, end_array),
                    setup=setup_code, globals={**_NS})

Note: The `process` function in this module benefits from the efficiency of NumPys element-wise
array operations, as it uses the `greater` and `multiply` functions to perform the required
operations on the input array.
"""

from numpy import ndarray, multiply, greater


def setup() -> str:
    """
    Set up code for greater multiply method.

    Returns:
        str: Setup code as a string.
    """
    return '''
from numpy import multiply, greater
'''


def process(start_array: ndarray, end_array: ndarray) -> ndarray:
    """
    Iterate through the start_array using element-wise array operations and apply filter and
    operation.
    ...
    Benefits: Uses NumPy's element-wise array operations to perform the required operations on
    the input array, avoiding explicit loops and making it more efficient.
    """
    end_array[:, :, 0] = multiply(greater(start_array, 0), start_array) * 255
    return end_array
