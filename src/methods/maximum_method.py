# src/methods/maximum_method.py
"""
maximum_method.py: A module that implements the maximum method for processing 2D arrays.
...
Example usage:
    ...
    from src.methods import maximum_method

    start_array = ...  # Your 2D input array
    end_array = ...    # Your 3D output array

    # Process the input array using the maximum method
    processed_array = maximum_method.process(start_array, end_array)

    # Benchmark the performance of the maximum method (timeit)
    timer = Timer(lambda: process(start_array, end_array), setup=setup_code, globals={**_NS})

Note: The `process` function in this module benefits from the efficiency of NumPy's `maximum`
...
def setup() -> str:
    """
    Set up code for maximum method.
...
def process(start_array: ndarray, end_array: ndarray) -> ndarray:
    """
    Iterate through the start_array using the maximum function and apply filter and operation.
    ...
    Benefits: Can be more efficient due to the use of NumPy's `maximum` function; no explicit loops
    required.
    """
    end_array[:, :, 0] = maximum(start_array, 0) * 255
    return end_array
