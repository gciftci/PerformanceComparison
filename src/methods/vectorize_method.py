# src/methods/vectorize_method.py
"""
vectorize_method.py: A module that implements the vectorize method for processing 2D arrays.
...
Example usage:
    ...
    from src.methods import vectorize_method

    start_array = ...  # Your 2D input array
    end_array = ...    # Your 3D output array

    # Process the input array using the vectorize method
    processed_array = vectorize_method.process(start_array, end_array)

    # Benchmark the performance of the vectorize method (timeit)
    timer = Timer(lambda: process(start_array, end_array), setup=setup_code, globals={**_NS})

Note: The `process` function in this module benefits from the convenience and readability of using
NumPy's vectorize function, but may not always be the most efficient method.
"""
...
def setup() -> str:
    """
    Set up code for vectorize method.
...
def custom_function(value):
    return 255 * value if value > 0 else 0
...
def process(start_array: ndarray, end_array: ndarray) -> ndarray:
    """
    Iterate through the start_array using vectorize and apply filter and operation.
    ...
    """
    vectorized_function = vectorize(lambda value: 255 * value if value > 0 else 0)
    res_array = vectorized_function(start_array)
    end_array[:, :, 0] = res_array
    return end_array
